...
- **Running head** on every page but the title page: `Surname / KEYWORD / page`. The surname is yours to state — left blank it's the last word of your name, which is right for most and wrong for every surname of more than one word. The settings page shows what it will print, so a bad guess is caught before an editor sees it a dozen times.
